...
class FinancialClearance(Document):
	...
	def on_submit(self):
		...
		frappe.db.sql(""" update `tabBuying Order` set financial_clearance_status = '{status}' where `tabBuying Order`.name = '{name}'  """.format(status= self.workflow_state, name=self.buying_order))
	...
	def on_cancel(self):
		...
		frappe.db.sql(""" update `tabBuying Order` set financial_clearance_status = "" where `tabBuying Order`.name = '{name}'  """.format( name=self.buying_order))
	@frappe.whitelist()
	def validate(doc, method=None):
		if not doc.technical_clearance_sessions:
			team = frappe.db.sql(""" select a.rank, a.name1, a.adjective
												from `tabTechnical clearance sessions` a join `tabTechnical Clearance` b
												on a.parent = b.name
												where b.buying_order = '{name}'
											""".format(name=doc.buying_order), as_dict=1)

			for z in team :
				teams = doc.append("technical_clearance_sessions", {})
				teams.rank = z.rank
				teams.name1 = z.name1
				teams.adjective = z.adjective
