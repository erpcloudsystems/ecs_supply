...
def get_columns(filters):
    if filters.get("recruit") and filters.get("filter") is None:
        columns = [
            ...
            {
                "label": _("رقم الصادر الأمن الجنائي" ),
                "fieldname": "send_investigation_number",
                "fieldtype": "Data",
                "width": 200
            },
            {
                "label": _(" تاريخ الصادر الأمن الجنائي"),
                "fieldname": "criminal_investigation_date",
                "fieldtype": "Date",
                "width": 200
            },
            {
                "label": _(" رقم الوارد الأمن الجنائي"),
                "fieldname": "investigation_entry_number",
                "fieldtype": "Data",
                "width": 200
            },
            {
                "label": _(" تاريخ الوارد الأمن الجنائي"),
        ...
        if "احكام افراد" in filter_option:
            return [
                {
                "label": _("المستند"),
                "fieldname": "docname",
        ...
        elif "معلومات افراد" in filter_option:
            return [
                {
                "label": _("المستند"),
                "fieldname": "docname",
...
def get_emps(filters):
    ...
    if filters.get("filter"):
        if "احكام افراد" in filters.get("filter"):
            conditions += " AND CPI.custom_criminal_investigation_result3 IS NOT NULL AND CPI.custom_criminal_investigation_result3  NOT LIKE '%يستدل%'  "
        elif "احكام الاقارب" in filters.get("filter"):
            conditions += " AND CPI.result_of_the_investions IS NOT NULL AND CPI.result_of_the_investions  NOT LIKE '%يستدل%'"
        elif "معلومات افراد" in filters.get("filter"):
            conditions += " AND CPI.criminal_investigation_result IS NOT NULL AND CPI.criminal_investigation_result  NOT LIKE '%يستدل%'"
        elif "معلومات الاقارب" in filters.get("filter"):
            conditions += " AND CPI.result_of_the_investions_of_his_relatives IS NOT NULL AND CPI.result_of_the_investions_of_his_relatives  NOT LIKE '%يستدل%'"
