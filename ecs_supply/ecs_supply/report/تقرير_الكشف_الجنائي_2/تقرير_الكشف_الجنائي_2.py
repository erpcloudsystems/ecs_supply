...
def get_columns(filters):
    if filters.get("the_recruits") and filters.get("filter") is None:
        columns = [
            {
                "label": _("المستند"),
                "fieldname": "docname",
        ...
        if "احكام مجندين" in filter_option:
            return [
                {
                "label": _("المستند"),
                "fieldname": "docname",
        ...
        elif "معلومات مجندين" in filter_option:
            return [
                {
                "label": _("المستند"),
                "fieldname": "docname",
...
def get_emps(filters):
    ...
    if filters.get("the_recruits"):
        conditions += f" AND CPI.the_recruits = '{filters['the_recruits']}'"
        values["the_recruits"] = filters["the_recruits"]
    ...
    for item_dict in basic_data:
        ...
        if not filters.get("filter"):
            first_table =frappe.db.sql("""
                ...
                FROM
                    `tabCriminal Security For Recruits` TROR
            ...
            second_table = frappe.db.sql("""
                SELECT
                    ...
                    TRJT.acts AS acts_1
                FROM
                    `tabThe Result Of The Recruiter Is Judgments Table` TRJT
                WHERE
                    TRJT.parent = %(name)s
            """, values,as_dict=1)
            
            if second_table and "احكام مجندين" in filters.get("filter"):
            ...
            third_table=frappe.db.sql("""
                SELECT
                    ...
                    RTI.acts AS acts_2
                FROM
                    `tabResult Of The Investions` RTI               
                WHERE
                    RTI.parent = %(name)s
            """, values,as_dict=1)
            ...
            forth_table=frappe.db.sql("""
                SELECT
                    ...
                    RIT.acts AS acts_4
                FROM
                    `tabThe Result Of The Recruiter Information Table` RIT
                                        
                WHERE
                    RIT.parent = %(name)s
            """, values,as_dict=1)
            if forth_table and "معلومات مجندين" in filters.get("filter"):
            ...
            fifth_table=frappe.db.sql("""
                SELECT              
                    ...
                    RIHT.acts AS acts_5
                FROM
                    `tabResult Of The investions Of His Relatives` RIHT
                WHERE
                    RIHT.parent = %(name)s
            """, values,as_dict=1)
