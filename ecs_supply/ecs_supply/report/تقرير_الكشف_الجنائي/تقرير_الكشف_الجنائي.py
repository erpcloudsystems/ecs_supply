...
def get_emps(filters):
    ...
    if filters.get("recruit"):
        conditions += f" AND EMP.first_name = '{filters['recruit']}' OR EMP.name = '{filters['recruit']}'  OR EMP.employee_number = '{filters['recruit']}'"
    ...
    basic_data = frappe.db.sql("""
        ...
        ON EMP.employee_number = CPI.the_one_who_is_looking_for OR EMP.first_name = CPI.the_one_who_is_looking_for OR EMP.name = CPI.the_one_who_is_looking_for 
        WHERE EMP.enabled =1
        {conditions}
    ...
    for item_dict in basic_data:
        ...
        if not filters.get("filter"):
            first_table =frappe.db.sql("""
                ...
                FROM
                    `tabCriminal Security For Employee` TROR
            ...
            second_table = frappe.db.sql("""
                SELECT
                    ...
                    TRJT.acts AS acts_1
                FROM
                    `tabThe Result Of The Individual Is Judgments Table` TRJT
                WHERE
                    TRJT.parent = %(name)s
            """, values,as_dict=1)
            
            if second_table and "احكام افراد" in filters.get("filter"):
            ...
            third_table=frappe.db.sql("""
                SELECT
                    ...
                    RTI.acts AS acts_2
                FROM
                    `tabThe Result Of His Relatives Is Judgments` RTI               
                WHERE
                    RTI.parent = %(name)s
            """, values,as_dict=1)
            
            if third_table is not None and "احكام الاقارب" in filters.get("filter"):
            ...
            forth_table=frappe.db.sql("""
                SELECT
                    ...
                    RIT.acts AS acts_4
                FROM
                    `tabThe Individual Results In Information` RIT
                                        
                WHERE
                    RIT.parent = %(name)s
            """, values,as_dict=1)
            if forth_table and "معلومات افراد" in filters.get("filter"):
            ...
            fifth_table=frappe.db.sql("""
                SELECT              
                    ...
                    RIHT.acts AS acts_5
                FROM
                    `tabHis Relatives Produce Information` RIHT
                WHERE
                    RIHT.parent = %(name)s
            """, values,as_dict=1)
