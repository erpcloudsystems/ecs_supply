...
def execute(filters=None):
    ...
    header = ""
    statistics = "<b><div style='text-align:center; background-color:lightblue; color:darkblue; display:flex; justify-content:space-around;'><div>عدد الضباط : {0}</div>".format(data[0]["total_officers"])
    statistics += "<div>عدد الضباط المدرجين : {0}</div>".format(data[0]["listed_officers"])
    statistics += "<div> عدد الضباط الغير مدرجين : {0}</div>".format(data[0]["unlisted_officers"])
    statistics += "<div> عدد الضباط المدرجين وتم طباعتهم : {0}</div>".format(data[0]["printed_officers"])
    statistics += "<div> عدد الضباط المدرجين ولم يتم طباعتهم : {0}</div></div></b>".format(data[0]["unprinted_officers"])
...
def get_columns():
    return [
        ...
        {
            "label": _("<b><p style='text-align:center; background-color:#005393; color:white'>نوع الوظيفـة</p></b>"),
            "fieldname": "job",
            "fieldtype": "Data",
            "width": 90
        },       
...
def get_item_price_qty_data(filters):
    ...
    if filters.get("officer_name"):
        conditions += """ and officer_name like '%{0}%' """.format(filters.get("officer_name"))
    ...
    if filters.get("job"):
        conditions += """ and job  = '{0}' """.format(filters.get("job"))
    ...
    counter = 0
    data = frappe.db.sql("""
		select name,officer_name,id_number2,rank,entities,reason_for_entitlement,job,assigned_work,print,print_date,fiscal_year
		from
    ...
    for row in data:
        counter += 1
        name = "<p style='text-align:right'><a href=/app/civilian-clothing-for-officers/{0}>{1}</a></p>".format(row.name,row.officer_name)
        id_number = "<p style='text-align:right'>{0}</p>".format(row.id_number2)
        ...
        job = "<p style='text-align:right'>{0}</p>".format(row.job)
